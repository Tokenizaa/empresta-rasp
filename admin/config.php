...
if (isset($_POST['salvar_config'])) {
    ...
    $revshare_padrao = str_replace(',', '.', $_POST['revshare_padrao']);

    // Novos campos de controle de jogo
    $chance_loss_percent = (int) $_POST['chance_loss_percent'];
    $max_prize_percent = (int) $_POST['max_prize_percent'];
    ...
    // Query atualizada para incluir os novos controles
    $stmt = $pdo->prepare("UPDATE config SET nome_site = ?, logo = ?, deposito_min = ?, saque_min = ?, cpa_padrao = ?, revshare_padrao = ?, chance_loss_percent = ?, max_prize_percent = ?");
    if ($stmt->execute([$nome_site, $logo, $deposito_min, $saque_min, $cpa_padrao, $revshare_padrao, $chance_loss_percent, $max_prize_percent])) {
                        ...
                        <div class="form-grid">
                            <div class="form-group">
                                <label class="form-label">
                                    ...
                                    Chance de Derrota Forçada (%)
                                </label>
                                <div class="input-container">
                                    <input type="number" name="chance_loss_percent"
                                        value="<?= htmlspecialchars($config['chance_loss_percent'] ?? '70') ?>"
                                        class="form-input" min="0" max="100" required>
                                    <span class="percentage-symbol">%</span>
                                </div>
                                <p style="color: #6b7280; font-size: 0.8rem; margin-top: 0.5rem;">
                                    <i class="fas fa-info-circle"></i>
                                    Defina a porcentagem de vezes que a casa deve ganhar (Padrão: 70%)
                                </p>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="input-container">
                                    <input type="number" name="max_prize_percent"
                                        value="<?= htmlspecialchars($config['max_prize_percent'] ?? '30') ?>"
                                        class="form-input" min="1" max="100" required>
                                    <span class="percentage-symbol">%</span>
                                </div>
                                <p style="color: #6b7280; font-size: 0.8rem; margin-top: 0.5rem;">
                                    <i class="fas fa-info-circle"></i>
                                    Máximo que o usuário pode ganhar baseado no saldo dele (Padrão: 30%)
                                </p>
                            </div>
                        </div>
                    </div>
