...
if (isset($_POST['aprovar_saque'])) {
    ...
    try {
        ...
        // Buscar dados do saque com bloqueio para escrita
        ...
        $stmt = $pdo->prepare("SELECT client_id, client_secret FROM silverpay LIMIT 1");
        $stmt->execute();
        $silverpay = $stmt->fetch();
        
        if (!$silverpay) {
            throw new Exception("Credenciais SilverPay não configuradas.");
        }
        
        $apiUrl = 'https://silverpay.io/v3/pix/payment';
        
        // Preparar o payload para a API da SilverPay
        $postData = [
            ...
            'chave_pix' => preg_replace('/\D/', '', $saque['cpf']), // Supondo que a chave pix é o CPF
        ];

        // Adicionando um log para debug
        file_put_contents('saques.log', date('[Y-m-d H:i:s] ') . "Tentativa de aprovar saque: " . json_encode($postData) . PHP_EOL, FILE_APPEND);
        
        // Configurar e executar a requisição cURL
        $ch = curl_init($apiUrl);
        ...
        file_put_contents('saques.log', date('[Y-m-d H:i:s] ') . "Resposta da API: HTTP $httpCode | Erro cURL: $curlError | Resposta: $response" . PHP_EOL, FILE_APPEND);
        ...
        // Lógica de verificação para o caso de a API retornar um array ou um objeto único
        $responseArray = [];
        if (isset($responseData[0])) {
            $responseArray = $responseData[0];
        } else {
            $responseArray = $responseData;
        }

        if ($httpCode !== 200 || !isset($responseArray['statusCode']) || $responseArray['statusCode'] != 200) {
            $errorMsg = "Falha ao processar pagamento na SilverPay. Resposta inesperada: " . $response;
            if (isset($responseArray['message'])) {
                $errorMsg = "Erro da API: " . $responseArray['message'];
            }
            throw new Exception($errorMsg);
        }

        // Se a requisição foi bem-sucedida, atualize o status do saque
        ...
        $pdo->commit();
        $_SESSION['success'] = 'Saque aprovado e pagamento realizado com sucesso via SilverPay!';
...
<body>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            // Fecha o loading caso ele tenha sido ativado antes do recarregamento
