...
// --- VERIFICAÇÃO DE SALDO NO GATEWAY (SILVERPAY) ---
// Evita liberar prêmios se a banca (Gateway) estiver sem fundos
require_once '../classes/SilverPay.php';
$gwStmt = $pdo->query("SELECT client_id, client_secret FROM silverpay LIMIT 1");
$gwCreds = $gwStmt->fetch(PDO::FETCH_ASSOC);

// Busca configurações de risco do banco de dados
$configStmt = $pdo->query("SELECT chance_loss_percent, max_prize_percent FROM config LIMIT 1");
$gameConfig = $configStmt->fetch(PDO::FETCH_ASSOC);

// Valores padrão caso não configurado
$chanceLoss = $gameConfig['chance_loss_percent'] ?? 70; // 70% de chance de perder
$maxPrizePct = $gameConfig['max_prize_percent'] ?? 30;  // 30% do saldo

// Define limite de prêmio baseado na % configurada do saldo do usuário
$maxPrizeAllowed = $usuario['saldo'] * ($maxPrizePct / 100);
$forceLoss = false;

// 1. REGRA: A CADA GUAL % DAS VEZES (Configurável)
// Se chance_loss for 70, então se der <= 70 (70% das vezes), perde.
if (mt_rand(1, 100) <= $chanceLoss) {
    $forceLoss = true;
}

// Tenta consultar o saldo (com timeout curto para não travar o jogo)
if ($gwCreds && !empty($gwCreds['client_id'])) {
    try {
        $silverPay = new SilverPay($gwCreds['client_id'], $gwCreds['client_secret']);
        $gatewayBalance = $silverPay->getBalance();

        if ($gatewayBalance !== null) {
            // Se a banca estiver muito baixa (< 20), trava tudo
            if ($gatewayBalance < 20.00) {
                $forceLoss = true;
            }

            // Segurança extra: O prêmio também não pode ser maior que o dinheiro que a casa tem
            // (Matém a lógica do usuário de 30% do saldo dele, mas se a banca tiver menos que isso, limita pela banca)
            if ($maxPrizeAllowed > $gatewayBalance) {
                $maxPrizeAllowed = $gatewayBalance;
            }
        }
    } catch (Exception $e) {
        error_log("Erro ao checar saldo SilverPay: " . $e->getMessage());
    }
}
// ----------------------------------------------------
...
// --- FILTRO DE SEGURANÇA DO GATEWAY ---
if ($forceLoss) {
    // Se forçado a perder (saldo < 20), remove todos os prêmios maiores que 0
    $premiosBrutos = array_filter($premiosBrutos, function ($p) {
        return (float) $p['valor'] <= 0;
    });
} else {
    // Remove prêmios maiores que o saldo disponível (regra de 80%)
    $premiosBrutos = array_filter($premiosBrutos, function ($p) use ($maxPrizeAllowed) {
        return (float) $p['valor'] <= $maxPrizeAllowed;
    });
}
// Re-indexa o array para evitar problemas no JSON ou loops
$premiosBrutos = array_values($premiosBrutos);
// ---------------------------------------

if (count($premiosBrutos) === 0) {
    http_response_code(500);
    exit(json_encode(['error' => 'Nenhum prêmio disponível (Verifique saldo do Gateway ou Configurações)']));
}
...
function aplicarBonusInfluencer(array $premios, float $custoRaspadinha, int $userId): array
{
    ...
    foreach ($ultimasJogadas as $index => $resultado) {
        if ($resultado === 'gain') {
            $vitoriasUltimas5++;
            if ($index === 0)
                break; // Se a última foi vitória, para a contagem de derrotas
        } else {
            if ($index < 3)
                $derrotasConsecutivas++; // Só conta derrotas recentes
        }
    }
